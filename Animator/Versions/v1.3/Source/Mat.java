...
public Mat multiply(Mat multiplyWith){
	...
	for(int i = 0; i < 3; i++){
		Vec v = new Vec(
			vectors[i].dotProduct(multiplyWith.getColumn(0)),
			vectors[i].dotProduct(multiplyWith.getColumn(1)),
			vectors[i].dotProduct(multiplyWith.getColumn(2)));
		product.setRow(v, i);
	}
	return product;
}
...
/*given a rotation matrix, extracts the degrees of
rotation the matrix is rotating*/
public static double extractRotation(Mat rot){
	/*this position in the rotation matrix is
	computed as Math.cos(rads)*/
	double value = rot.getRow(0).getPoint(0);
	value = Math.acos(value);
	return Math.toDegrees(value);
}

/*given a translation matrix, extracts the x and y
distances of translation*/
public static double[] extractTranslation(Mat trans){
	double[] translation = new double[2];
	translation[0] = trans.getRow(0).getPoint(2);
	translation[1] = trans.getRow(1).getPoint(2);
	return translation;
}

/*given a scalar matrix, extracts the x and y
weights of the scale*/
public static double[] extractScale(Mat scale){
	double[] s = new double[2];
	s[0] = scale.getRow(0).getPoint(0);
	s[1] = scale.getRow(1).getPoint(1);
	return s;
}
...
/*Returns the greatest vec[0](x value), greatest vec[1](y value),
smallest vec[0](x value), or smallest vec[1](y value) amoung any
of the vectors in this matrix, based on the operation given*/
public double getExtreames(int operation){
	double ex = 0;
	if(operation == 0){ //greatest x value
		ex = vectors[0].getPoint(0);
		for(int i = 1; i < size(); i++)
			if(ex < vectors[i].getPoint(0))
				ex = vectors[i].getPoint(0);
	}
	else if(operation == 1){ //greatest y value
		ex = vectors[0].getPoint(1);
		for(int i = 1; i < size(); i++)
			if(ex < vectors[i].getPoint(1))
				ex = vectors[i].getPoint(1);
	}
	else if(operation == 2){ //smallest x value
		ex = vectors[0].getPoint(0);
		for(int i = 1; i < size(); i++)
			if(ex > vectors[i].getPoint(0))
				ex = vectors[i].getPoint(0);
	}
	else{ //smallest y value
		ex = vectors[0].getPoint(1);
		for(int i = 1; i < size(); i++)
			if(ex > vectors[i].getPoint(1))
				ex = vectors[i].getPoint(1);
...
/*Returns the index in this matrix of the following from:
	0  1  2
	3  4  5
	6  7  8*/
