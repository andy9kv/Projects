...
/*close to a default constructor, expect initializes the name*/
public Node(String name){ this(name, 0); }

/*initializes name and uniqueID to the specified name and uniqueID*/
public Node(String name, int uniqueID){ 
	this(new Poly(), name, uniqueID);
}

/*initializes the Poly to the specified Poly, the name to the
specified name, and identity matrices for transformations*/
public Node(Poly shape, String name){ this(shape, name, 0); }
...
/*initializes a mostly empty node with the specifed children and
 specified name*/
public Node(ArrayList<Node> children, String name){
	this(children, name, 0);
}
...
public Node(Node original){
	...
	this.rot = new Mat(original.getRotation());
	this.trans = new Mat(original.getTranslation());
	this.scale = new Mat(original.getScale());
}
...
/*rather than simply set the rotation matrix, this function adds
the rotation to the previously existing rotation matrix*/
public void addRotation(double angle){
	Mat r = Mat.rotationMatrix(angle);
	this.rot = r.multiply(getRotation());
}

/*rather than simply set the translation matrix, this function adds
the translation to the previously existing translation matrix*/
public void addTranslation(double x, double y){
	Mat t = Mat.translationMatrix(x, y);
	this.trans = t.multiply(getTranslation());
}

/*rather than simply set the scale matrix, this function adds
the scale to the previously existing scalar matrix*/
public void addScale(double x, double y){
	Mat s = Mat.scaleMatrix(x, y);
	this.scale = s.multiply(getScale());
}
...
/*transformational matrices getters*/
public Mat getRotation(){ return this.rot;}
public Mat getTranslation(){ return this.trans;}
public Mat getScale(){ return this.scale;}

/*returns rotation value, not matrix (can be an approximation)*/
public double getRot(){ 
	Mat r = getRotation();
	return Mat.extractRotation(r);
}

/*returns translation values, not matrix ([x][y])*/
public double[] getTrans(){
	Mat t = getTranslation(); 
	return Mat.extractTranslation(t);
}

/*returns scalar values, not matrix ([x][y])*/
public double[] getScalar(){
	Mat s = getScale(); 
	return Mat.extractScale(s);
}
