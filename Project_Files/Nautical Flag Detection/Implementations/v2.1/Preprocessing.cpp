...
Mat Preprocessing::isolateColor(Mat img, Scalar color) {
	...
	if (sEquals(color, COLOR_WHITE)) {
		inRange(img, Scalar(100, 100, 100), Scalar(255, 255, 255), dest);
		...
		inRange(img, Scalar(5, 0, 0), Scalar(255, 60, 60), dest);
		out.setTo(COLOR_BLUE, dest);
	}
	else if (sEquals(color, COLOR_RED)){
		inRange(img, Scalar(0, 0, 50), Scalar(50, 50, 255), dest);
...
/*Returns an image with all given values in between min and max set to color */
Mat Preprocessing::isolateColor(Mat img, Scalar color, Scalar max, Scalar min) {
	Mat dest;
	Mat out = img.clone();
	out.setTo(0);
	inRange(img, min, max, dest);
	out.setTo(color, dest);
	return out;
}
...
/*returns the sum of both images*/
Mat Preprocessing::addImg(Mat a, Mat b) {
	Mat out = a.clone();
	add(out, b, out);
	return out;
}
...
bool Preprocessing::mEquals(Mat m1, Mat m2) {
	Mat diff = m1 != m2;
