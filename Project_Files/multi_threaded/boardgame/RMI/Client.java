...
public class Client {
	...
	public Message comm(ServerIF server, Message msg) throws Exception {
		msg.dateSent = new Date(System.currentTimeMillis());
		try{
			msg = server.sendMsg(msg);
		} catch (Exception e){
	...
	public void driver(Scanner sc) throws Exception {

		/* Connect to Server */
		ServerIF server = (ServerIF) Naming.lookup("MSG");
		...
		Message msg = new Message(width, height);
		msg = comm(server, msg);
		...
		while(!resp.equals("quit")){
			...
			switch (resp) {
				...
				case "up": /* move up */
				case "down": /* move down */
					msg = comm(server, new Message(resp));
					if(msg.response == 0){
